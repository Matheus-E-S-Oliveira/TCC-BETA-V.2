...
export class GraficoTelaGrandeService {

  constructor() { }

  tamanhoTela(){
    let quantidadeMonth: number = 0;
    if(window.innerWidth > 1000){
      return quantidadeMonth = 6;
    }
    else{
      return quantidadeMonth = 4;
    }
  }

  formatData(avaliacao: any[], currentMonthIndex: number): any {
    let quantidadeMonth: number = this.tamanhoTela();
    const totalMonths = avaliacao.length;
    let startIndex = Math.max(totalMonths - quantidadeMonth - currentMonthIndex, 0);
    const lastSixMonths = avaliacao.slice(startIndex, startIndex + quantidadeMonth);
  ...
  private createDatasets(categorias: any[], lastSixMonths: any[]): any[] {
    ...
    const mediaGeralDataset = {
      ...
      pointBorderColor: "rgba(150, 150, 150, 0.9)",
    };
